...
from app.models.reservoir import ReservoirVolume
from app.repositories.gcs_repository import GCSRepository
from app.core.ons_client import ONSClient
from app.core.exceptions import ONSClientError
from app.core.logging_decorator import logging_it
...
class ReservoirService:
    ...
    def _fetch_and_save_year(self, year: int) -> int:
        ...
        try:
            ...
            logging.warning(f"Could not process year {year}: {e}")
        return 0
    ...
    def ingest_data(self, start_date: date, end_date: date) -> int:
        ...
        with ThreadPoolExecutor(max_workers=5) as executor:
            ...
            total_rows_ingested = sum(results)
        
        if total_rows_ingested == 0:
            raise HTTPException(status_code=404, detail="No data found in ONS for the specified period.")
    ...
    def get_historical_volume(self, start_date: date, end_date: date, page: int, size: int) -> dict:
        """
        Retrieves historical reservoir data from the repository and formats it
        ...
        for index, row in result_df.iterrows():
            try:
                ...
                volume = ReservoirVolume.model_validate(row, from_attributes=True)
