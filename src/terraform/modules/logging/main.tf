# Cloud Logging sink to export logs to the Sauter University bucket
resource "google_logging_project_sink" "terraform_logs_sink" {
  name                   = var.log_sink_name
  destination            = "storage.googleapis.com/${var.terraform_logs_bucket_name}"
...
resource "google_storage_bucket_iam_member" "terraform_logs_sink_writer" {
  bucket = var.terraform_logs_bucket_name
